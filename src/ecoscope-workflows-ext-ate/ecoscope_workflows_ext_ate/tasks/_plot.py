...
class TukeyPlotStyle(BaseModel):
    ...
    confidence_level: Annotated[float | SkipJsonSchema[None], AdvancedField(ge=0.0, le=1.0, default=0.95)] = 0.95
...
def create_likert_chart(
    ...
    if sort_questions:
        df = _sort_dataframe(df, positive_responses, negative_responses, sort_by)
    ...
    for column in columns:
        try:
            ...
            pie_chart = draw_pie_chart(
                ...
                label_column=column,
                color_column="colors",
                plot_style=PlotStyle(textinfo="percent", marker_colors=column_colors),
            ...
            if pie_chart is None:
                ...
                file_path = persist_text(pie_chart, output_dir, f"{new_col}_pie_chart.html")
...
def draw_bar_and_persist(output_dir, df: AnyDataFrame, columns: Union[str, List[str]]):
    ...
    bar_dir_list = []
    bar_configs = [BarConfig(column="id", agg_func="count", label="", style=PlotCategoryStyle(marker_color="#7eb0d5"))]
    ...
    for column in columns:
        try:
            ...
            if bar_chart is None:
                ...
                file_path = persist_text(bar_chart, output_dir, f"{new_col}_bar_chart.html")
...
def draw_boxplot(
    ...
    y_column: Annotated[
        str,
        Field(description="The name of the dataframe column to use for box plot values (y-axis)."),
    ],
    x_column: Annotated[
    ...
    plot_style: Annotated[
        ...
        AdvancedField(default=None, description="Additional style kwargs passed to go.Box()."),
    ] = None,
    ...
    for column in columns:
        try:
            boxplot_chart = draw_boxplot(
                ...
                plot_style=PlotStyle(marker_colors=["#7eb0d5"]),
                layout_style=LayoutStyle(
                    showlegend=False,
                    bargap=0.1,
                    font_size=13,
            ...
            if boxplot_chart is None:
                ...
                file_path = persist_text(  # persist_text
                    boxplot_chart, output_dir, f"{new_col}_boxplot_chart.html"
                )
...
def draw_ols_scatterplot_and_persist(
    output_dir, df: AnyDataFrame, columns: Union[str, List[str]], y_column: str
) -> list:
    if df is None or df.empty:
        raise ValueError("DataFrame is None or empty")
    ...
    for column in columns:
        try:
            ...
            if scatter_chart is None:
                ...
                file_path = persist_text(  # persist_text
                    scatter_chart, output_dir, f"{new_col}_ols_scatter_chart.html"
                )
...
def draw_tukey_plot(
    ...
    group_data = [clean_df[clean_df[group_column] == group][value_column].values for group in groups]
    comparisons = []
    ...
    comparison_labels = [f"{comp['group1']} - {comp['group2']}" for comp in comparisons]
    traces = []
    for i, comp in enumerate(comparisons):
        color = tukey_style.significant_color if comp["is_significant"] else tukey_style.non_significant_color
        traces.append(
            go.Scatter(
                x=[comp["mean_diff"]],
                y=[i],
...
def draw_tukey_plots_and_persist(
    output_dir, df: AnyDataFrame, columns: Union[str, List[str]], value_column: str
) -> list:
    if df is None or df.empty:
        raise ValueError("DataFrame is None or empty")
    ...
    for column in columns:
        try:
            ...
            if tukey_chart is None:
                ...
                file_path = persist_text(  # persist_text
                    tukey_chart, output_dir, f"{new_col}_tukey_chart.html"
                )
